...
export class PromotionsService {
  ...
  async createClaim(user: Actor, input: ClaimInput) {
    ...
    if (linkedTopup) {
      const duplicateTopup = await this.findClaimUsingTopup(linkedTopup.id);
      if (duplicateTopup) throw new BadRequestException('รายการฝากนี้ถูกใช้รับโปรไปแล้ว');
    }
  ...
  private async findClaimUsingTopup(topupId: string) {
    ...
    return claims.find((claim) => this.claimMetadata(claim.metadata).topupId === topupId) ?? null;
  }
