...
export class TopUpsService {
  ...
  async approveRequest(id: string, adminUser: any, dto: ReviewTopUpRequestDto, meta: RequestMeta = {}) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!request) throw new NotFoundException('Top up request not found');
      if (request.status !== 'PENDING') throw new ConflictException('Top up request already reviewed');

      let wallet = await tx.wallet.findUnique({ where: { userId: request.userId } });
      if (!wallet) {
        wallet = await tx.wallet.create({ data: { userId: request.userId, currency: request.currency } });
      }
      ...
      await tx.adminAuditLog.create({
        data: {
          ...
          newData: { status: updated.status, balanceBefore: balanceBefore.toString(), balanceAfter: balanceAfter.toString() } as any,
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.adminAuditLog.create({
        data: {
          ...
          targetId: request.id,
          oldData: { status: request.status, amount: request.amount.toString() } as any,
          newData: { status: updated.status, adminNote: dto.adminNote } as any,
          ipAddress: meta.ipAddress,
          userAgent: meta.userAgent,
        },
      });
