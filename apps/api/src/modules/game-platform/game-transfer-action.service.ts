...
export class GameTransferActionService {
  ...
  async manualReverse(id: string, actor: AdminActor, note: string) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const ledger = await tx.walletLedger.create({ data: { walletId: wallet.id, userId: transfer.userId, type: 'REVERSAL', direction, amount: amount.toFixed(2), balanceBefore: before.toFixed(2), balanceAfter: after.toFixed(2), referenceType: 'GAME_TRANSFER_MANUAL_REVERSE', referenceId: transfer.id, idempotencyKey, metadata: { transferId: transfer.id, providerId: transfer.providerId, note, reversedBy: actor.id, reversedAt: new Date().toISOString() } } });
      const payload = this.mergeJson(transfer.responsePayload, { manualReverse: { note, reversedBy: actor.id, reversedAt: new Date().toISOString(), ledgerId: ledger.id, direction, balanceAfter: updatedWallet.balance } });
      const updatedTransfer = await tx.gameTransfer.update({ where: { id: transfer.id }, data: { status: 'REVERSED', responsePayload: payload, resolvedAt: new Date() }, include: { user: { select: { id: true, username: true, phone: true } }, provider: { select: { id: true, name: true, code: true } }, session: { select: { id: true, providerSessionId: true, game: { select: { id: true, name: true, providerGameCode: true } } } } } });
      await tx.adminAuditLog.create({ data: { adminUserId: actor.id, action: 'game.transfer.manual_reverse', module: 'game-platform', targetId: transfer.id, newData: this.safeJson({ note, ledgerId: ledger.id, direction, balanceAfter: updatedWallet.balance }) } });
  ...
  async forceFail(id: string, actor: AdminActor, note: string) {
    const transfer = await this.prisma.gameTransfer.findUnique({ where: { id } });
    if (!transfer) throw new NotFoundException('Game transfer not found');
    if (transfer.status === 'SUCCESS' || transfer.status === 'REVERSED') throw new BadRequestException('SUCCESS/REVERSED transfer cannot be force failed');
    const payload = this.mergeJson(transfer.responsePayload, { forceFail: { note, failedBy: actor.id, failedAt: new Date().toISOString() } });
    const item = await this.prisma.gameTransfer.update({ where: { id }, data: { status: 'FAILED', errorCode: transfer.errorCode ?? 'FORCE_FAILED', errorMessage: note, responsePayload: payload, resolvedAt: new Date() }, include: { user: { select: { id: true, username: true, phone: true } }, provider: { select: { id: true, name: true, code: true } }, session: { select: { id: true, providerSessionId: true, game: { select: { id: true, name: true, providerGameCode: true } } } } } });
    await this.prisma.adminAuditLog.create({ data: { adminUserId: actor.id, action: 'game.transfer.force_fail', module: 'game-platform', targetId: id, newData: this.safeJson({ note }) } });
