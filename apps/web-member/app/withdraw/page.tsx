    ...
    <main style={pageStyle}>
      ...
      {step === 'confirm' && (
        <section style={cardStyle}>
          ...
          <div style={actionRowStyle}><button type="button" onClick={() => setStep('amount')} style={secondaryButtonStyle}>แก้ไข</button><button type="button" onClick={submit} disabled={isSubmitting} style={buttonStyle}>{isSubmitting ? 'กำลังส่ง...' : 'ยืนยันถอนเงิน'}</button></div>
        </section>
      )}
...
const stepperStyle = { border: '1px solid rgba(255,255,255,.10)', borderRadius: 22, padding: 8, background: 'rgba(255,255,255,.045)', display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 6, minWidth: 0 } as const;
