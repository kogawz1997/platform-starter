...
export default function MemberSignInPage() {
  ...
  useEffect(() => {
    if (window.localStorage.getItem('member_access_token')) {
  ...
  const loginEnabled = boolSetting(settings, 'features', 'login_enabled', true) && boolSetting(settings, 'website', 'login_enabled', true);
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!loginEnabled) { setStatus('error'); setMessage('ขณะนี้ปิดเข้าสู่ระบบ'); return; }
    if (!identifier.trim() || !secret.trim()) { setStatus('error'); setMessage('กรุณากรอก Username / Phone / Email และรหัสผ่าน'); return; }

    setLoading(true); setStatus('info'); setMessage('กำลังตรวจสอบข้อมูล...');
    ...
    if (!res.ok) { setStatus('error'); setMessage(data?.message ?? 'เข้าสู่ระบบไม่สำเร็จ กรุณาตรวจสอบข้อมูล'); return; }
    ...
    setStatus('success'); setMessage('เข้าสู่ระบบสำเร็จ กำลังพาไปหน้าแรก...');
    const next = new URLSearchParams(window.location.search).get('next') || '/';
    ...
    <main style={{ ...pageStyle, background: backgroundColor, color: textColor }}>
      <section style={shellStyle}>
        <div style={heroStyle}>
          ...
          <div style={featureStyle}><span>กระเป๋าเงิน</span><span>ประวัติธุรกรรม</span><span>ฝาก/ถอน</span></div>
        </div>

        <form onSubmit={onSubmit} style={{ ...cardStyle, background: cardColor }}>
          <div><p style={{ margin: 0, opacity: 0.7 }}>เข้าสู่ระบบสมาชิก</p><h2 style={{ margin: '6px 0 0' }}>{siteName}</h2></div>
          {(maintenanceEnabled || !loginEnabled) && <div style={alertStyle('error')}>{maintenanceEnabled ? 'ระบบกำลังปรับปรุง' : 'ขณะนี้ปิดเข้าสู่ระบบ'}</div>}
          <label style={labelStyle}>Username / Phone / Email<input value={identifier} onChange={(e) => setIdentifier(e.target.value)} placeholder="เช่น Th11 หรือเบอร์โทร" disabled={disabled} autoComplete="username" style={inputStyle} /></label>
          <label style={labelStyle}>รหัสผ่าน<div style={passwordWrapStyle}><input value={secret} onChange={(e) => setSecret(e.target.value)} placeholder="กรอกรหัสผ่าน" type={showSecret ? 'text' : 'password'} disabled={disabled} autoComplete="current-password" style={{ ...inputStyle, paddingRight: 84 }} /><button type="button" onClick={() => setShowSecret((v) => !v)} style={ghostButtonStyle} disabled={disabled}>{showSecret ? 'ซ่อน' : 'แสดง'}</button></div></label>
          <button type="submit" disabled={disabled} style={{ ...submitStyle, background: primaryColor, color: '#111' }}>{loading ? 'กำลังเข้าสู่ระบบ...' : 'เข้าสู่ระบบ'}</button>
          {message && <div style={alertStyle(status)}>{message}</div>}
          <p style={{ textAlign: 'center', margin: 0 }}>ยังไม่มีบัญชี? <a href="/register" style={{ color: primaryColor }}>สมัครสมาชิก</a></p>
        </form>
      </section>
    </main>
  );
