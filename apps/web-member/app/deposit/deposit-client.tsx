...
export default function DepositClient() {
  ...
  const [message, setMessage] = useState('กำลังโหลดข้อมูลฝากเงิน...');
  ...
  async function nextStep(event: FormEvent<HTMLFormElement>) {
    ...
    setLoading(true); setMessage('กำลังเตรียมข้อมูลรับเงิน...');
  ...
  async function uploadSlip(event: ChangeEvent<HTMLInputElement>) {
    ...
    const imageData = await resizeImage(file, 900, 0.72); setSlipImageName(file.name); setSlipImageData(imageData); setMessage('แนบสลิปแล้ว ตรวจรูปอีกครั้งก่อนส่ง');
  }
  ...
  async function submit() {
    ...
    setHistory((current) => [data, ...current]); setSlipImageData(''); setSlipImageName(''); setNote(''); setMessage('ส่งรายการแล้ว รอแอดมินตรวจสอบ'); setStep('waiting');
  }

  return <main style={pageStyle}><a href="/" style={backStyle}>← หน้าแรก</a><h1 style={titleStyle}>ฝากเงิน</h1><p style={mutedStyle}>เลือกยอดเงินและวิธีการโอน จากนั้นทำรายการตามข้อมูลที่ระบบแสดง</p>{message && <div style={noticeStyle}>{message}</div>}{step === 'select' && <form onSubmit={nextStep} style={cardStyle}><h2 style={sectionHeadingStyle}>เลือกยอดเงิน</h2><div style={amountGridStyle}>{AMOUNTS.map((value) => <button key={value} type="button" onClick={() => setAmount(String(value))} style={amountButtonStyle(Number(amount) === value)}>฿{value.toLocaleString('th-TH')}</button>)}</div><label style={labelStyle}>จำนวนเงิน<input value={amount} onChange={(e) => setAmount(e.target.value)} inputMode="decimal" style={inputStyle} /></label><h2 style={sectionHeadingStyle}>วิธีการโอน</h2>{(['bank_transfer', 'promptpay', 'wallet', 'other'] as MethodCode[]).map((code) => { const enabled = usable.some((account) => accountType(account) === code); return <button key={code} type="button" disabled={!enabled} onClick={() => enabled && setMethod(code)} style={methodStyle(method === code, enabled)}><strong>{METHODS[code].label}</strong><span>{enabled ? 'พร้อมใช้งาน' : 'ยังไม่เปิดใช้งาน'}</span></button>; })}<button type="submit" disabled={loading} style={primaryButtonStyle}>{loading ? 'กำลังเตรียม...' : 'ถัดไป'}</button></form>}{step === 'transfer' && selected && <section style={cardStyle}><h2 style={sectionHeadingStyle}>{METHODS[method].label}</h2><h1 style={amountTitleStyle}>฿{parsedAmount.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</h1><section style={boxStyle}><InfoRow label="ชื่อบัญชี" value={selected.accountName} /><InfoRow label={METHODS[method].numberLabel} value={selected.accountNumber} action={<button type="button" onClick={() => copyText(selected.accountNumber, METHODS[method].numberLabel)} style={copyButtonStyle}>คัดลอก</button>} />{selected.promptPay && <InfoRow label="PromptPay" value={selected.promptPay} action={<button type="button" onClick={() => copyText(selected.promptPay ?? '', 'พร้อมเพย์')} style={copyButtonStyle}>คัดลอก</button>} />}{selected.qrImageUrl && <img src={selected.qrImageUrl} alt="QR" style={qrStyle} />}<p style={mutedStyle}>โอนเสร็จแล้วแนบสลิป แล้วกดส่งรายการ</p></section><label style={labelStyle}>แนบสลิป<input type="file" accept="image/*" onChange={uploadSlip} style={inputStyle} /></label>{slipImageData && <section style={previewBoxStyle}><strong>ตัวอย่างสลิปที่แนบ</strong><img src={slipImageData} alt="slip" style={slipStyle} /></section>}<label style={labelStyle}>หมายเหตุ<textarea value={note} onChange={(e) => setNote(e.target.value)} style={textareaStyle} placeholder="รายละเอียดเพิ่มเติม ถ้ามี" /></label><div style={actionRowStyle}><button type="button" onClick={() => setStep('select')} style={secondaryButtonStyle}>ย้อนกลับ</button><button type="button" onClick={submit} disabled={loading || !slipImageData} style={primaryButtonStyle}>{loading ? 'กำลังส่ง...' : 'ส่งรายการ'}</button></div></section>}{step === 'waiting' && <section style={cardStyle}><h2 style={sectionHeadingStyle}>รอแอดมินอนุมัติ</h2><p style={mutedStyle}>ส่งรายการแล้ว กรุณารอตรวจสอบ</p><button type="button" onClick={() => setStep('select')} style={secondaryButtonStyle}>ฝากเพิ่มอีกครั้ง</button></section>}<h2 style={sectionHeadingStyle}>ประวัติคำขอ</h2>{history.map((item) => <section key={item.id} style={historyCardStyle}><div style={historyTopStyle}><strong>{item.currency} {Number(item.amount).toLocaleString('th-TH', { minimumFractionDigits: 2 })}</strong><span style={statusBadgeStyle(item.status)}>{item.status}</span></div><p style={mutedStyle}>Method: {item.method ?? '-'}</p><p style={mutedStyle}>{new Date(item.createdAt).toLocaleString('th-TH')}</p>{item.adminNote && <p style={mutedStyle}>Admin note: {item.adminNote}</p>}</section>)}</main>;
}
...
const primaryButtonStyle = { padding: 14, minHeight: 48, borderRadius: 16, cursor: 'pointer', background: '#f5c542', color: '#111', border: 0, fontWeight: 900, width: '100%', opacity: 1 } as const;
...
const amountTitleStyle = { margin: 0, fontSize: 'clamp(32px,10vw,52px)', lineHeight: 1, overflowWrap: 'anywhere' as const, color: '#f5c542' };
...
function statusBadgeStyle(status: string) { return { width: 'fit-content', borderRadius: 999, padding: '6px 10px', background: status === 'APPROVED' ? 'rgba(80,255,140,.14)' : status === 'REJECTED' ? 'rgba(255,80,80,.14)' : 'rgba(245,197,66,.14)', border: '1px solid rgba(255,255,255,.12)', fontSize: 12, fontWeight: 900 }; }
