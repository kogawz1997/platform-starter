...
export default function DepositPage() {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(async () => {
      ...
      if (!token) return;
      const res = await fetch(`${API_URL}/member/topups/${pendingRequest.id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  async function loadHistory() {
    ...
    const res = await fetch(`${API_URL}/member/topups`, { headers: { Authorization: `Bearer ${token}` } });
    const data = await res.json().catch(() => null);
  ...
  async function handleSlipChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    const imageData = await resizeImage(file, 420, 0.35);
  ...
  async function submitAfterTransfer() {
    ...
    const res = await fetch(`${API_URL}/member/topups`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ amount: finalAmount, method, note: proofNote }) });
    const data = await res.json().catch(() => null);
  ...
  return (
    <main style={{ maxWidth: 980, margin: '32px auto', padding: 24 }}>
      ...
      {step === 'select' && <form onSubmit={goToTransfer} style={panelStyle}>
        <h2>เลือกจำนวนเงิน</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: 10 }}>{AMOUNTS.map((value) => <button key={value} type="button" onClick={() => { setAmount(String(value)); setSelectedAmount(value); }} style={amountButtonStyle(parseAmount(amount) === value)}>฿{value.toLocaleString('th-TH')}</button>)}</div>
        <label>หรือใส่จำนวนเอง<input value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="เช่น 500" inputMode="decimal" style={inputStyle} /></label>
        <h2>เลือกวิธีการเติม</h2>
        <div style={{ display: 'grid', gap: 10 }}>{METHODS.map((item) => <button key={item.code} type="button" onClick={() => setMethod(item.code)} style={methodButtonStyle(method === item.code)}><strong>{item.label}</strong><span>{item.detail}</span></button>)}</div>
        <button type="submit" style={primaryButtonStyle}>เสร็จสิ้น</button>{message && <p>{message}</p>}
      </form>}
      {step === 'transfer' && <section style={panelStyle}>
        <h2>{selectedMethod.title}</h2><div style={summaryStyle}><p>ยอดที่ต้องโอน</p><h1>฿{transferAmount.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</h1><p>ช่องทาง: {selectedMethod.label}</p></div>
        {method === 'bank_transfer' && <section style={paymentBoxStyle}><strong>ข้อมูลบัญชี</strong><p>ชื่อบัญชี: Platform Demo</p><p>เลขบัญชี: 000-000-0000</p><p>ธนาคาร: Demo Bank</p><p>โอนเสร็จแล้วให้แนบรูปสลิป แล้วกด “เสร็จ”</p></section>}
        {method === 'qr_promptpay' && <section style={paymentBoxStyle}><strong>QR Code</strong><div style={qrBoxStyle}>QR</div><p>สแกน QR แล้วโอนตามยอด ฿{transferAmount.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</p></section>}
        <label>แนบสลิปโอนเงิน<input type="file" accept="image/*" onChange={handleSlipChange} style={inputStyle} /></label>
        {slipImageData && <section style={previewBoxStyle}><strong>ตัวอย่างสลิป</strong><img src={slipImageData} alt="slip preview" style={{ width: '100%', maxWidth: 320, borderRadius: 12, marginTop: 10 }} /><p>{slipImageName}</p></section>}
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><button type="button" onClick={() => setStep('select')} style={secondaryButtonStyle}>ย้อนกลับ</button><button type="button" onClick={submitAfterTransfer} disabled={isSubmitting} style={primaryButtonStyle}>{isSubmitting ? 'กำลังส่ง...' : 'เสร็จ'}</button></div>{message && <p>{message}</p>}
      </section>}
      {step === 'waiting' && <section style={panelStyle}><h2>รอแอดมินอนุมัติ</h2><p>รายการของคุณถูกส่งแล้ว สถานะตอนนี้: {pendingRequest?.status ?? 'PENDING'}</p><p>หน้านี้จะเช็กสถานะให้อัตโนมัติทุก 5 วินาที</p><div style={summaryStyle}><p>ยอดรายการ</p><h1>฿{Number(pendingRequest?.amount ?? selectedAmount).toLocaleString('th-TH', { minimumFractionDigits: 2 })}</h1></div>{message && <p>{message}</p>}</section>}
      {step === 'approved' && <section style={panelStyle}><h2>ทำรายการเสร็จสิ้น ✅</h2><p>แอดมินอนุมัติแล้ว ยอดจะถูกเพิ่มเข้ากระเป๋าเงิน</p><a href="/" style={primaryLinkStyle}>กลับหน้าหลัก</a>{message && <p>{message}</p>}</section>}
      {step === 'rejected' && <section style={panelStyle}><h2>รายการไม่ผ่าน</h2><p>{message || 'กรุณาตรวจสอบข้อมูลแล้วทำรายการใหม่'}</p><button type="button" onClick={() => setStep('select')} style={primaryButtonStyle}>ทำรายการใหม่</button></section>}
      <h2>ประวัติคำขอ</h2><div style={{ display: 'grid', gap: 12 }}>{items.map((item) => { const proof = parseProofNote(item.note); return <section key={item.id} style={{ border: '1px solid #ddd', borderRadius: 14, padding: 16 }}><strong>{item.currency} {Number(item.amount).toLocaleString('th-TH', { minimumFractionDigits: 2 })}</strong><p>Status: {item.status}</p><p>สลิป: {proof.slipImageData ? 'แนบแล้ว' : '-'}</p><p>หมายเหตุ: {proof.userNote || '-'}</p>{item.adminNote && <p>Admin note: {item.adminNote}</p>}</section>; })}{items.length === 0 && <p>ยังไม่มีรายการ</p>}</div>
    </main>
  );
}
...
function stepItemStyle(active: boolean) { return { padding: 10, borderRadius: 12, textAlign: 'center' as const, border: '1px solid #ddd', background: active ? '#0a84ff' : '#fff', color: active ? '#fff' : '#111' }; }
function amountButtonStyle(active: boolean) { return { padding: 12, borderRadius: 12, border: active ? '2px solid #0a84ff' : '1px solid #ddd', cursor: 'pointer', background: active ? '#e8f2ff' : '#fff' }; }
function methodButtonStyle(active: boolean) { return { display: 'grid', gap: 4, textAlign: 'left' as const, padding: 14, borderRadius: 12, border: active ? '2px solid #0a84ff' : '1px solid #ddd', cursor: 'pointer', background: active ? '#e8f2ff' : '#fff' }; }
