...
type BonusLedger = { id: string; campaignId: string; campaign?: { title?: string }; amount: number; currency: string; turnoverRequired: number; turnoverProgress: number; turnoverCompleted: boolean; walletCreditEnabled: boolean; walletCreditStatus: string; status: string; events?: Array<{ by: string; action: string; amount?: number; message?: string; createdAt: string }>; createdAt: string; resolvedAt?: string | null };
...
export default function MemberBonusPage() {
  ...
  useEffect(() => { load(); }, []);
  const stats = useMemo(() => ({ total: items.reduce((sum, item) => sum + Number(item.amount || 0), 0), active: items.filter((item) => item.status === 'ACTIVE').length, completed: items.filter((item) => item.turnoverCompleted).length, remaining: items.reduce((sum, item) => sum + Math.max(Number(item.turnoverRequired || 0) - Number(item.turnoverProgress || 0), 0), 0) }), [items]);
  ...
  return <main style={pageStyle}><section style={heroStyle}><span style={eyebrowStyle}>Bonus</span><h1 style={titleStyle}>โบนัสของฉัน</h1><p style={mutedStyle}>ดูยอดโบนัสและเงื่อนไขเทิร์น ตอนนี้โบนัสยังไม่ถูกเพิ่มเข้า wallet หลักจนกว่า guard จะพร้อม</p><div style={statGridStyle}><Stat label="ยอดโบนัส" value={money(stats.total)} /><Stat label="ใช้งาน" value={String(stats.active)} /><Stat label="ผ่านเทิร์น" value={String(stats.completed)} /><Stat label="เทิร์นคงเหลือ" value={money(stats.remaining)} /></div></section>{message && <div style={noticeStyle}>{message}</div>}<section style={listStyle}>{items.map((item) => <article key={item.id} style={cardStyle}><div style={rowStyle}><strong>{item.campaign?.title ?? item.campaignId}</strong><span style={badgeStyle}>{statusLabel(item.status)}</span></div><p style={mutedStyle}>โบนัส {money(item.amount)} · {item.currency}</p><div style={conditionGridStyle}><Condition label="เทิร์นแล้ว" value={money(item.turnoverProgress)} /><Condition label="ต้องทำ" value={money(item.turnoverRequired)} /><Condition label="คงเหลือ" value={money(Math.max(item.turnoverRequired - item.turnoverProgress, 0))} /></div><div style={progressOuterStyle}><div style={{ ...progressInnerStyle, width: `${progressPercent(item)}%` }} /></div><small style={mutedStyle}>{item.walletCreditStatus}</small></article>)}{items.length === 0 && <div style={emptyStyle}>ยังไม่มีโบนัส</div>}</section><MemberBottomNav /></main>;
}
...
function statusLabel(status: string) { const map: Record<string, string> = { ACTIVE: 'กำลังทำเทิร์น', REVIEWING: 'กำลังตรวจ', TURNOVER_COMPLETED: 'ผ่านเทิร์นแล้ว', COMPLETED: 'เสร็จสิ้น', EXPIRED: 'หมดอายุ' }; return map[status] ?? status; }
