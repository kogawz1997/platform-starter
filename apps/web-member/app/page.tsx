...
import { boolSetting, defaultSettings, loadPublicSiteSettings, PublicSiteSettings, textSetting } from './site-settings';
...
export default function Page() {
  ...
  const description = textSetting(settings, 'website', 'site_description', 'แพลตฟอร์มสมาชิกครบวงจร ฝาก ถอน โปรโมชั่น และกิจกรรม');
  const primaryColor = textSetting(settings, 'branding', 'primary_color', '#b516ff');
  const backgroundColor = textSetting(settings, 'branding', 'background_color', '#0b0712');
  const cardColor = textSetting(settings, 'branding', 'card_color', '#171021');
  ...
  return (
    <main className="casino-member-page" style={{ minHeight: '100vh', background: backgroundColor, color: textColor, overflowX: 'hidden' }}>
      <MemberHome siteName={siteName} description={description} primaryColor={primaryColor} cardColor={cardColor} textColor={textColor} showBalanceHeader={showBalanceHeader} showButtons={showButtons} showPromotion={showPromotion} showCategories={showCategories} showProviders={showProviders} showRecommended={showRecommended} />
    </main>
  );
}
