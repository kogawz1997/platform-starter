...
export default function MemberGamesPage() {
  ...
  async function launchGame(game: Game) { setLaunching({ gameId: game.id, message: `กำลังเปิด ${game.name}...` }); const res = await memberApiFetch(`/member/games/${game.id}/launch`, { method: 'POST' }); const data = await res.json().catch(() => null); if (!res.ok || !data?.ok || !data?.launchUrl) { setLaunching({ gameId: game.id, message: data?.message ?? data?.errorMessage ?? 'เปิดเกมไม่สำเร็จ' }); return; } setLaunching({ gameId: game.id, message: 'เปิดเกมแล้ว' }); window.location.href = data.launchUrl; }
  return <main style={pageStyle}><section style={heroStyle}><span style={eyebrowStyle}>Game Lobby</span><h1 style={titleStyle}>เลือกเกม</h1><p style={mutedStyle}>รวมเกมที่เปิดใช้งานแล้วจาก catalog ระบบ รอบนี้เป็น launch dry-run ยังไม่ตัดเงินจริง</p></section>{message && <div style={noticeStyle}>{message}</div>}{launching.message && <div style={noticeStyle}>{launching.message}</div>}<div style={tabsStyle}><button style={category === 'all' ? activeTabStyle : tabStyle} onClick={() => setCategory('all')}>ทั้งหมด</button>{(payload.categories ?? []).map((item) => <button key={item} style={category === item ? activeTabStyle : tabStyle} onClick={() => setCategory(item)}>{item}</button>)}</div><GameSection title="เกมแนะนำ" items={payload.featured ?? []} launchingGameId={launching.gameId} onLaunch={launchGame} /><GameSection title="มาใหม่" items={payload.newest ?? []} launchingGameId={launching.gameId} onLaunch={launchGame} /><GameSection title="ยอดนิยม" items={payload.popular ?? []} launchingGameId={launching.gameId} onLaunch={launchGame} /><section style={sectionStyle}><h2 style={sectionTitleStyle}>เกมทั้งหมด</h2><div style={gridStyle}>{filtered.map((game) => <GameCard key={game.id} game={game} launching={launching.gameId === game.id} onLaunch={launchGame} />)}{!message && filtered.length === 0 && <div style={emptyStyle}>ยังไม่มีเกมในหมวดนี้</div>}</div></section></main>;
}

function GameSection({ title, items, launchingGameId, onLaunch }: { title: string; items: Game[]; launchingGameId?: string; onLaunch: (game: Game) => void }) { if (items.length === 0) return null; return <section style={sectionStyle}><h2 style={sectionTitleStyle}>{title}</h2><div style={gridStyle}>{items.slice(0, 8).map((game) => <GameCard key={game.id} game={game} launching={launchingGameId === game.id} onLaunch={onLaunch} />)}</div></section>; }
function GameCard({ game, launching, onLaunch }: { game: Game; launching: boolean; onLaunch: (game: Game) => void }) { const image = pickImage(game); return <article style={cardStyle}>{image ? <img src={image} alt="" style={imageStyle} /> : <div style={fallbackStyle}>{game.name.slice(0, 2).toUpperCase()}</div>}<div style={cardBodyStyle}><strong>{game.name}</strong><span>{game.provider?.name ?? game.providerGameCode}</span><div style={badgeRowStyle}>{game.isFeatured && <em>แนะนำ</em>}{game.isNew && <em>ใหม่</em>}{game.isPopular && <em>นิยม</em>}</div><button type="button" style={playButtonStyle} onClick={() => onLaunch(game)} disabled={launching}>{launching ? 'กำลังเปิด...' : 'เล่น'}</button></div></article>; }
...
const playButtonStyle = { marginTop: 4, minHeight: 36, borderRadius: 12, border: 0, background: '#f5c542', color: '#111827', fontWeight: 950 } as const;
