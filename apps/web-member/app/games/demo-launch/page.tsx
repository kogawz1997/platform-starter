...
export default function DemoLaunchPage() {
  ...
  async function transfer(type: 'transfer-in' | 'transfer-out') {
    ...
    setBusy('');
    if (!res.ok || !data?.ok) {
      setMessage(data?.transfer?.errorMessage ?? data?.message ?? data?.errorMessage ?? 'โยกเงินไม่สำเร็จ');
      await loadAll();
      return;
    }
  ...
  return <main style={pageStyle}>
    <section style={cardStyle}>
      <span style={eyebrowStyle}>Wallet Sync Game Session</span>
      <h1 style={titleStyle}>โยกเงินเข้าออกเกม</h1>
      <p style={mutedStyle}>เงินจะ sync กับวอเลตจริง: โยกเข้าเกมจะหักยอดวอเลต, โยกออกจะบวกกลับเข้า wallet พร้อมบันทึก ledger</p>
      ...
      <section style={walletCardStyle}>
        <span>ยอดวอเลต</span>
        <strong>{formatMoney(wallet.balance ?? '0.00', wallet.currency ?? 'THB')}</strong>
        <small>ยอดที่ถูกพัก: {formatMoney(wallet.lockedBalance ?? '0.00', wallet.currency ?? 'THB')}</small>
      </section>
      <section style={panelStyle}>
        <strong>โยกเงิน</strong>
        <p style={mutedStyle}>ใส่จำนวนเงินแล้วเลือกว่าจะโยกเข้าเกมหรือโยกกลับเข้าวอเลต ถ้า provider fail ระบบจะ rollback ตาม flow ที่ควรเป็น ไม่ใช่ปล่อยเงินหายไปเดินเล่น</p>
...
const screenStyle = { minHeight: 160, borderRadius: 22, border: '1px solid rgba(148,163,184,.2)', background: 'linear-gradient(135deg,rgba(245,197,66,.16),rgba(59,130,246,.12))', display: 'grid', placeItems: 'center', textAlign: 'center' as const, gap: 8, padding: 18 };
