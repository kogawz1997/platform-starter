  ...
  return (
    <main style={pageStyle}>
      ...
      <section style={listStyle}>
        {items.map((item) => <div key={item.id} style={cardStyle}><div style={rowStyle}><div style={infoStyle}><span style={badgeStyle(item.status)}>{item.status}</span><h2 style={bankTitleStyle}>{item.bankName}</h2><p style={mutedStyle}>{item.accountName}</p><p style={mutedStyle}>{item.accountNumber}</p>{item.adminNote && <p style={mutedStyle}>Admin note: {item.adminNote}</p>}</div><div style={actionStyle}><strong>{item.isPrimary ? 'บัญชีหลัก' : ''}</strong>{!item.isPrimary && <button type="button" onClick={() => setPrimary(item.id)} style={secondaryButtonStyle}>ตั้งเป็นหลัก</button>}</div></div></div>)}
...
const secondaryButtonStyle = { marginTop: 10, padding: '10px 12px', minHeight: 42, borderRadius: 999, border: '1px solid rgba(255,255,255,0.16)', background: 'rgba(255,255,255,0.08)', color: '#fff', fontWeight: 900, cursor: 'pointer', width: '100%' } as const;
...
const actionStyle = { textAlign: 'right' as const, minWidth: 0, flex: '1 1 140px' };
const bankTitleStyle = { margin: '10px 0 4px', overflowWrap: 'anywhere' as const };
