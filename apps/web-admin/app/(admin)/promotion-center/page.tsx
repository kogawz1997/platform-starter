...
export default function PromotionCenterPage() {
  ...
  useEffect(() => { load(); }, []);
  const stats = useMemo(() => ({ enabled: campaigns.filter((item) => item.enabled).length, total: campaigns.length, manual: campaigns.filter((item) => item.claimMode === 'manual_review').length, warnings: buildWarnings(campaigns).length }), [campaigns]);
  ...
  async function save() { setSaving(true); setMessage('กำลังบันทึกโปรโมชัน...'); const res = await adminApiFetch('/admin/settings/features', { method: 'PUT', body: JSON.stringify({ promotion_campaigns: campaigns }) }); const data = await res.json().catch(() => null); setSaving(false); if (!res.ok) { setMessage(data?.message ?? 'บันทึกโปรโมชันไม่สำเร็จ'); return; } setMessage('บันทึกโปรโมชันแล้ว ยังไม่เปิด settlement เงินจริงจนกว่าจะทำ bonus ledger'); }
  const warnings = buildWarnings(campaigns);
  return <AdminPage eyebrow="Growth" title="Promotion / Bonus" description="ตั้งค่าโปรโมชันให้สมาชิกเห็นได้จริง แต่ยังไม่เพิ่มยอดโบนัสเข้ากระเป๋าจริงจนกว่า bonus ledger และ audit guard จะพร้อม" actions={<><AdminButton tone="secondary" onClick={load}>รีเฟรช</AdminButton><AdminButton onClick={save} disabled={saving}>{saving ? 'กำลังบันทึก...' : 'บันทึก'}</AdminButton></>}>
    {message && <AdminNotice>{message}</AdminNotice>}
    {warnings.length > 0 && <AdminNotice>{warnings.join(' • ')}</AdminNotice>}
    <AdminMetricGrid><AdminMetric tone={stats.enabled > 0 ? 'success' : 'warning'} title="โปรที่เปิด" value={String(stats.enabled)} helper={`${stats.total} รายการ`} /><AdminMetric tone="warning" title="Claim mode" value={`${stats.manual} manual`} helper="ยังไม่ auto settlement" /><AdminMetric tone="danger" title="Bonus wallet" value="Off" helper="ยังไม่เขียนเงินจริง" /><AdminMetric tone={stats.warnings > 0 ? 'danger' : 'success'} title="Warnings" value={String(stats.warnings)} /></AdminMetricGrid>
    <AdminGrid><AdminCard title="Campaigns" tone="warning"><AdminStack>{campaigns.map((item, index) => <CampaignEditor key={item.id} item={item} index={index} count={campaigns.length} onPatch={(patch) => patchCampaign(index, patch, setCampaigns)} onRemove={() => setCampaigns((current) => current.filter((_, i) => i !== index))} onMove={(direction) => moveCampaign(index, direction, setCampaigns)} />)}<AdminButton tone="secondary" onClick={() => setCampaigns((current) => [...current, createCampaign(current.length + 1)])}>เพิ่มโปรโมชัน</AdminButton></AdminStack></AdminCard><AdminCard title="Member preview" description="ตัวอย่างที่สมาชิกจะเห็น"><div style={previewShellStyle}>{campaigns.filter((item) => item.enabled).slice(0, 3).map((item) => <section key={item.id} style={previewCardStyle}><AdminBadge tone="warning">{item.bonusType === 'percent' ? `${item.bonusValue}%` : money(item.bonusValue)}</AdminBadge><h3>{item.title}</h3><p>{item.description}</p><span>ฝากขั้นต่ำ {money(item.minDeposit)} · เทิร์น x{item.turnoverMultiplier}</span></section>)}{campaigns.filter((item) => item.enabled).length === 0 && <AdminNotice>ยังไม่มีโปรที่เปิดให้สมาชิกเห็น</AdminNotice>}</div></AdminCard></AdminGrid>
    <AdminCard title="Guard ก่อนเปิดเงินจริง" tone="danger"><AdminStack><AdminRow><strong>Bonus ledger</strong><AdminBadge tone="danger">ยังไม่เปิด</AdminBadge></AdminRow><AdminRow><strong>Turnover tracking</strong><AdminBadge tone="warning">ต้องทำต่อ</AdminBadge></AdminRow><AdminRow><strong>Claim audit</strong><AdminBadge tone="warning">ต้องทำต่อ</AdminBadge></AdminRow><AdminRow><strong>Auto settlement</strong><AdminBadge tone="danger">ปิดไว้ก่อน</AdminBadge></AdminRow></AdminStack></AdminCard>
...
function CampaignEditor({ item, index, count, onPatch, onRemove, onMove }: { item: PromotionCampaign; index: number; count: number; onPatch: (patch: Partial<PromotionCampaign>) => void; onRemove: () => void; onMove: (direction: -1 | 1) => void }) { return <section style={editorStyle}><AdminRow><div><strong>{item.title || `Promotion ${index + 1}`}</strong><p style={mutedStyle}>{item.enabled ? 'เปิดให้สมาชิกเห็น' : 'ปิดอยู่'}</p></div><div style={actionRowStyle}><AdminBadge tone={item.enabled ? 'success' : 'neutral'}>{item.enabled ? 'เปิด' : 'ปิด'}</AdminBadge>{index > 0 && <AdminButton tone="secondary" onClick={() => onMove(-1)}>ขึ้น</AdminButton>}{index < count - 1 && <AdminButton tone="secondary" onClick={() => onMove(1)}>ลง</AdminButton>}<AdminButton tone="secondary" onClick={() => onPatch({ enabled: !item.enabled })}>{item.enabled ? 'ปิด' : 'เปิด'}</AdminButton><AdminButton tone="danger" onClick={onRemove}>ลบ</AdminButton></div></AdminRow><div style={fieldGridStyle}><Field label="รหัสโปร" value={item.id} onChange={(value) => onPatch({ id: slug(value) })} /><Field label="ชื่อโปร" value={item.title} onChange={(value) => onPatch({ title: value })} /><Field label="รายละเอียด" value={item.description} onChange={(value) => onPatch({ description: value })} textarea /><label style={fieldStyle}><span>ประเภทโบนัส</span><select value={item.bonusType} onChange={(event) => onPatch({ bonusType: event.target.value as PromotionCampaign['bonusType'] })} style={inputStyle}><option value="percent">เปอร์เซ็นต์</option><option value="fixed">จำนวนคงที่</option></select></label><NumberField label="โบนัส" value={item.bonusValue} onChange={(value) => onPatch({ bonusValue: value })} /><NumberField label="ฝากขั้นต่ำ" value={item.minDeposit} onChange={(value) => onPatch({ minDeposit: value })} /><NumberField label="โบนัสสูงสุด" value={item.maxBonus} onChange={(value) => onPatch({ maxBonus: value })} /><NumberField label="เทิร์น x" value={item.turnoverMultiplier} onChange={(value) => onPatch({ turnoverMultiplier: value })} /><label style={fieldStyle}><span>Claim mode</span><select value={item.claimMode} onChange={(event) => onPatch({ claimMode: event.target.value as PromotionCampaign['claimMode'] })} style={inputStyle}><option value="manual_review">แอดมินตรวจ</option><option value="auto_pending">สร้างคำขอรอตรวจอัตโนมัติ</option></select></label><Field label="เริ่ม" value={item.startsAt ?? ''} onChange={(value) => onPatch({ startsAt: value || undefined })} /><Field label="สิ้นสุด" value={item.endsAt ?? ''} onChange={(value) => onPatch({ endsAt: value || undefined })} /></div></section>; }
...
function createCampaign(index: number): PromotionCampaign { return { id: `promotion-${Date.now()}`, title: `โปรโมชัน ${index}`, description: '', enabled: false, bonusType: 'percent', bonusValue: 10, minDeposit: 100, maxBonus: 500, turnoverMultiplier: 3, claimMode: 'manual_review' }; }
function patchCampaign(index: number, patch: Partial<PromotionCampaign>, setCampaigns: React.Dispatch<React.SetStateAction<PromotionCampaign[]>>) { setCampaigns((current) => current.map((item, i) => i === index ? { ...item, ...patch } : item)); }
function moveCampaign(index: number, direction: -1 | 1, setCampaigns: React.Dispatch<React.SetStateAction<PromotionCampaign[]>>) { setCampaigns((current) => { const next = [...current]; const target = index + direction; if (target < 0 || target >= next.length) return current; [next[index], next[target]] = [next[target], next[index]]; return next; }); }
function buildWarnings(campaigns: PromotionCampaign[]) { const warnings = []; const ids = new Set<string>(); for (const item of campaigns) { if (!item.id.trim()) warnings.push('มีโปรที่ไม่มีรหัส'); if (ids.has(item.id)) warnings.push(`รหัสโปรซ้ำ: ${item.id}`); ids.add(item.id); if (item.enabled && !item.title.trim()) warnings.push('มีโปรเปิดอยู่แต่ไม่มีชื่อ'); if (item.bonusType === 'percent' && item.bonusValue > 100) warnings.push('เปอร์เซ็นต์โบนัสเกิน 100%'); if (item.maxBonus < 0 || item.minDeposit < 0 || item.bonusValue < 0) warnings.push('ตัวเลขโปรโมชันต้องไม่ติดลบ'); } return warnings; }
...
const previewCardStyle = { border: '1px solid rgba(245,197,66,.24)', borderRadius: 18, padding: 12, background: 'rgba(245,197,66,.08)', display: 'grid', gap: 6 } as const;
