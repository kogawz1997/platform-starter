...
const THAI_BANKS = ['ธนาคารกสิกรไทย', 'ธนาคารไทยพาณิชย์', 'ธนาคารกรุงเทพ', 'ธนาคารกรุงไทย', 'ธนาคารกรุงศรีอยุธยา', 'ธนาคารทหารไทยธนชาต', 'ธนาคารออมสิน', 'ธนาคารอาคารสงเคราะห์', 'ธนาคารเพื่อการเกษตรและสหกรณ์การเกษตร', 'ธนาคารยูโอบี', 'ธนาคารซีไอเอ็มบีไทย', 'ธนาคารเกียรตินาคินภัทร', 'ธนาคารแลนด์ แอนด์ เฮ้าส์', 'ธนาคารไอซีบีซี ไทย', 'ธนาคารไทยเครดิต', 'พร้อมเพย์'];
...
export default function BankAccountsPage() {
  ...
  return (
    <AdminPage eyebrow="Bank Operations" title="Bank Accounts" description="จัดการบัญชีรับเงิน QR Code และตรวจบัญชีถอนเงินของสมาชิก" actions={<AdminButton onClick={loadAll}>Refresh</AdminButton>}>
      {message && <AdminNotice>{message}</AdminNotice>}
      <AdminGrid>
        <AdminCard title="Add Receiving Bank" description="บัญชีนี้จะแสดงในหน้าฝากเงินของสมาชิก">
          <form onSubmit={saveReceiving}>
            <AdminToolbar>
              <label style={labelStyle}>ธนาคาร<select value={form.bankName} onChange={(e) => setForm({ ...form, bankName: e.target.value })}>{THAI_BANKS.map((bank) => <option key={bank} value={bank}>{bank}</option>)}</select></label>
              <label style={labelStyle}>ชื่อบัญชีรับเงิน<input value={form.accountName} onChange={(e) => setForm({ ...form, accountName: e.target.value })} placeholder="ชื่อบัญชี" /></label>
              <label style={labelStyle}>เลขบัญชี / เบอร์พร้อมเพย์<input value={form.accountNumber} onChange={(e) => setForm({ ...form, accountNumber: e.target.value })} placeholder="Account number" /></label>
              <label style={labelStyle}>PromptPay<input value={form.promptPay} onChange={(e) => setForm({ ...form, promptPay: e.target.value })} placeholder="PromptPay optional" /></label>
              ...
              <AdminButton type="submit">Add Bank</AdminButton>
            </AdminToolbar>
          </form>
        </AdminCard>
        <AdminCard title="Receiving Banks"><AdminStack>{receiving.map((item) => <AdminRow key={item.id}><div><AdminBadge tone={item.status === 'ACTIVE' ? 'success' : 'danger'}>{item.status}</AdminBadge><h2 style={{ margin: '10px 0 4px' }}>{item.bankName}</h2><p>{item.accountName} / {item.accountNumber}</p><p>PromptPay: {item.promptPay ?? '-'}</p><p>Limit: {item.minAmount ?? '-'} - {item.maxAmount ?? '-'}</p>{item.qrImageUrl && <img src={item.qrImageUrl} alt="QR" style={qrPreviewStyle} />}</div><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><AdminButton tone="success" disabled={busyId === item.id} onClick={() => setReceivingStatus(item, 'ACTIVE')}>Enable</AdminButton><AdminButton tone="danger" disabled={busyId === item.id} onClick={() => setReceivingStatus(item, 'DISABLED')}>Disable</AdminButton></div></AdminRow>)}{receiving.length === 0 && <AdminEmpty>ยังไม่มีบัญชีรับเงิน</AdminEmpty>}</AdminStack></AdminCard>
      </AdminGrid>
