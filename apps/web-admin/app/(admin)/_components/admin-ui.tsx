...
export function AdminCard({ title, description, action, children }: CardProps) {
  return <div style={cardStyle}>{(title || description || action) && <div style={cardHeadStyle}><div style={{ minWidth: 0 }}>{title && <h2 style={cardTitleStyle}>{title}</h2>}{description && <p style={mutedStyle}>{description}</p>}</div>{action && <div style={cardActionStyle}>{action}</div>}</div>}<div style={stackStyle}>{children}</div></div>;
}
...
const pageStyle = { width: '100%', maxWidth: 1180, margin: '0 auto', padding: '24px 20px 56px', color: '#f8fafc', minWidth: 0, overflowX: 'hidden' as const } as const;
const pageHeadStyle = { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 16, marginBottom: 18, flexWrap: 'wrap' as const, minWidth: 0 };
...
const titleStyle = { margin: '4px 0 10px', fontSize: 'clamp(30px, 6vw, 48px)', lineHeight: 1.02, letterSpacing: -0.9, fontWeight: 950, overflowWrap: 'anywhere' as const };
...
const cardStyle = { border: '1px solid rgba(148,163,184,.18)', borderRadius: 18, background: '#111a24', padding: 16, boxShadow: 'none', minWidth: 0, overflow: 'hidden' as const };
const cardHeadStyle = { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', marginBottom: 12, flexWrap: 'wrap' as const, minWidth: 0 };
const cardActionStyle = { display: 'flex', gap: 8, flexWrap: 'wrap' as const, justifyContent: 'flex-end' as const, maxWidth: '100%' };
const cardTitleStyle = { margin: 0, fontSize: 22, lineHeight: 1.18, fontWeight: 900, overflowWrap: 'anywhere' as const };
const mutedStyle = { margin: '5px 0 0', color: '#94a3b8', lineHeight: 1.55 } as const;
const metricGridStyle = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(180px, 100%), 1fr))', gap: 12, marginBottom: 16, minWidth: 0 };
const metricStyle = { border: '1px solid rgba(148,163,184,.18)', borderRadius: 16, background: '#111a24', padding: 16, minWidth: 0, overflow: 'hidden' as const };
const metricLabelStyle = { margin: '0 0 8px', color: '#94a3b8', fontSize: 13 } as const;
const metricValueStyle = { display: 'block', fontSize: 'clamp(22px,4vw,30px)', lineHeight: 1.05, overflowWrap: 'anywhere' as const };
...
const buttonBaseStyle = { minHeight: 44, borderRadius: 12, border: '1px solid rgba(148,163,184,.18)', padding: '0 14px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, cursor: 'pointer', maxWidth: '100%', textAlign: 'center' as const, whiteSpace: 'normal' as const };
...
const badgeStyle = { display: 'inline-flex', alignItems: 'center', minHeight: 26, borderRadius: 999, padding: '0 10px', fontSize: 12, fontWeight: 950, border: '1px solid rgba(148,163,184,.18)', maxWidth: '100%', overflowWrap: 'anywhere' as const };
