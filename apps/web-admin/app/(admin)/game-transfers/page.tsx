...
export default function GameTransfersPage() {
  ...
  async function retryTransfer(item: Transfer) { const note = window.prompt('Retry note'); if (!note) return; setWorking(item.id); const res = await adminApiFetch(`/admin/game-transfers/${item.id}/retry-dry-run`, { method: 'POST', body: JSON.stringify({ note }) }); const data = await res.json().catch(() => null); setWorking(''); if (!res.ok || !data?.ok) { setMessage(data?.message ?? data?.errorMessage ?? 'retry ไม่สำเร็จ'); return; } setMessage(`retry สำเร็จ: ${data.transfer?.id ?? '-'}`); await loadTransfers(); }
  return <AdminPage eyebrow="Game Platform" title="Game Transfers" description="ดู transfer log ที่ sync wallet จริง พร้อม link เข้า detail เพื่อตรวจ ledger/rollback" actions={<AdminButton onClick={loadTransfers} disabled={loading}>Refresh</AdminButton>}>
    <AdminMetricGrid><AdminMetric title="Transfers" value={String(metrics.total)} helper="latest 100" /><AdminMetric title="Success" value={String(metrics.success)} helper="wallet sync success" /><AdminMetric title="Pending" value={String(metrics.pending)} helper="waiting" /><AdminMetric title="Failed" value={String(metrics.failed)} helper="failed / rollback" /></AdminMetricGrid>
    ...
    <AdminStack>{filtered.map((item) => <AdminCard key={item.id}><AdminRow><div><h2 style={titleStyle}>{item.type} · {item.amount} {item.currency}</h2><p style={mutedStyle}>{item.provider?.name ?? '-'} · {item.session?.game?.name ?? '-'} · user {item.user?.username ?? item.user?.phone ?? '-'}</p><p style={smallMutedStyle}>idempotency: {item.idempotencyKey}</p><p style={smallMutedStyle}>providerTx: {item.providerTransactionId ?? '-'}</p></div><div style={badgeStackStyle}><AdminBadge tone={statusTone(item.status)}>{item.status}</AdminBadge><AdminLinkButton href={`/game-transfers/${item.id}`}>Detail</AdminLinkButton><AdminButton tone="secondary" onClick={() => setExpanded(expanded === item.id ? '' : item.id)}>{expanded === item.id ? 'Hide payload' : 'Payload'}</AdminButton><AdminButton tone="secondary" onClick={() => reviewTransfer(item)} disabled={working === item.id}>{working === item.id ? 'Working...' : 'Review'}</AdminButton>{item.status === 'FAILED' && <AdminButton tone="secondary" onClick={() => retryTransfer(item)} disabled={working === item.id}>Retry</AdminButton>}</div></AdminRow>{item.errorMessage && <AdminNotice>{item.errorCode ?? 'ERROR'}: {item.errorMessage}</AdminNotice>}{expanded === item.id && <pre style={preStyle}>{JSON.stringify({ requestPayload: item.requestPayload, responsePayload: item.responsePayload }, null, 2)}</pre>}<p style={smallMutedStyle}>created {new Date(item.createdAt).toLocaleString('th-TH')}</p></AdminCard>)}{!loading && filtered.length === 0 && <AdminEmpty>ยังไม่มี game transfer</AdminEmpty>}</AdminStack>
  </AdminPage>;
}
