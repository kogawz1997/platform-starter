...
export default function GameTransfersPage() {
  ...
  async function reviewTransfer(item: Transfer) { const note = window.prompt('Review note'); if (!note) return; setReviewing(item.id); const res = await adminApiFetch(`/admin/game-transfers/${item.id}/review`, { method: 'PATCH', body: JSON.stringify({ note }) }); const data = await res.json().catch(() => null); setReviewing(''); if (!res.ok) { setMessage(data?.message ?? 'review transfer ไม่สำเร็จ'); return; } setMessage('บันทึก review แล้ว'); await loadTransfers(); }
