...
export default function WalletLedgersPage() {
  ...
  useEffect(() => { load(); }, []);
  async function load() { setLoading(true); setMessage('กำลังโหลด wallet ledger...'); const res = await adminApiFetch('/admin/money-ops/ledger?take=100'); const data = await res.json().catch(() => null); setLoading(false); if (!res.ok) { setMessage(data?.message ?? 'โหลด wallet ledger ไม่สำเร็จ'); return; } setItems(data.items ?? []); setMessage(''); }
